...
import org.workflowsim.reclustering.ClusteringSizeEstimator;
 ...
 *
 */
public class CrowdReclusteringEngine {
     ...
     * @return a new job
     */
    private static CrowdTask createJob(int id, CrowdTask job, long length, List taskList, boolean updateDep) {
        try {
        	CrowdTask newJob = new CrowdTask(id, length);
     ...
     */
    public static List<CrowdTask> process(CrowdTask job, int id) {        
     ...
     * @return
     */
    private static List<CrowdTask> VerticalReclustering(List<CrowdTask> jobList, CrowdTask job, int id) {
        ...
        List<CrowdTask> newUpList = DCReclustering(new ArrayList(), job, id, listUp);
        id += newUpList.size();
     ...
     * @return
     */
    private static List BlockReclustering(List jobList, CrowdTask job, int id) {
     ...
     * @return
     */
    private static List<CrowdTask> DCReclustering(List<CrowdTask> jobList, CrowdTask job, int id, List<Task> allTaskList) {
        ...
        if (suggestedK == 0) {
            ...
            List<CrowdTask> retryJobs = new ArrayList<>();
            long length = 0;
            CrowdTask newJob = createJob(id, job, 0, null, false);
     ...
     * @return
     */
    private static List<CrowdTask> SRReclustering(List<CrowdTask> jobList, CrowdTask job, int id) {
     ...
     * @return
     */
    private static List DRReclustering(List<CrowdTask> jobList, CrowdTask job, int id, List<Task> allTaskList) {
        Task firstTask = allTaskList.get(0);
    ...
    private static void updateDependencies (CrowdTask job, List<CrowdTask> jobList) {
        ...
        for (Task childTask : children) {
            CrowdTask childJob = (CrowdTask) childTask;
