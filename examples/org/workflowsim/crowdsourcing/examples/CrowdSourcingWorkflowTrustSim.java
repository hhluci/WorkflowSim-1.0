...
public class CrowdSourcingWorkflowTrustSim {

	private static FileOutputStream fos;
	static {
		File file = new File("c://out"+DateUtil.getTodayDate()+".csv");
		try {
	...
	protected static List<CondorVM> createMachines(int userId, int vms) {
        ...
        for (int i = 0; i < vms; i++) {
            ...
            CondorVM vm = new Machine(i, userId, mips * ratio, pesNumber, ram, bw, size, 
    ...
    public static void main(String[] args) {
        try {
            ...
            int workerNum = 150; //number of workers
            int machineNum = 150;//number of machines
            /**
             ...
             * generate failures.
             */
            FailureParameters.FTCFailure ftc_failure = FailureParameters.FTCFailure.FAILURE_VM_TRUST;////FAILURE_NONE;//FAILURE_ALL;//
            /**
             ...
             */
            int maxLevel = 11;
            DistributionGenerator[][] failureGenerators = new DistributionGenerator[workerNum+machineNum][maxLevel];
            /**
             * Model failures as a function of VM (each VM has its own
             * independent distribution
             */
            for (int vmId = 0; vmId < workerNum+machineNum; vmId++) {
             ...
             */
            Parameters.SchedulingAlgorithm sch_method = Parameters.SchedulingAlgorithm.CROWD_TRUST;
            ...
            Parameters.init(machineNum+workerNum, daxPath, null,
            ...
            List<CondorVM> machinelist = createMachines(wfEngine.getSchedulerId(0), machineNum);
            List<CondorVM> workerlist = createWorkers(wfEngine.getSchedulerId(0), machineNum,workerNum);
    ...
    protected static void printVmList(List<CondorVM> condorVMs) {
    	for(CondorVM condorVm: condorVMs) {
    		if(condorVm instanceof Worker) {
    			...
    			int allocatedTaskNumber = worker.getAllocatedTaskNumber()-worker.getRawAllocatedTaskNumber();
    			int completedTaskNumber = worker.getCompletedTaskNumber()-worker.getRawcompletedTaskNumber();
    			Log.printLine("worker:"+worker.getId()+","
    			+allocatedTaskNumber+","
    					+completedTaskNumber);
    		}else if(condorVm instanceof Machine) {
    			Machine machine = (Machine)condorVm;
    			int allocatedTaskNumber = machine.getAllocatedTaskNumber()-machine.getRawAllocatedTaskNumber();
    			int completedTaskNumber = machine.getCompletedTaskNumber()-machine.getRawcompletedTaskNumber();
    			Log.printLine("machine:"+machine.getId()+","
    			+allocatedTaskNumber+","
    					+completedTaskNumber);
    		}
    	}
